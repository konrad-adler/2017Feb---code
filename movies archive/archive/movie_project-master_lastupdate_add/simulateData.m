...
n = 500;    % individuals per market
nmarket = 5;
nmovies = 10; 
ntaste = 2; % dimension of taste space
...
Setup.X_j = [...
log(round(unifrnd(80,120,nmovies,1))) ...% some running length
binornd(1,0.55,nmovies,1) ...% some dummy
log(exprnd(10,nmovies,1))]; % budget



% Movie unobservables/position
Setup.zeta_j = normrnd(2,0.25,nmovies,1)*0; 
%Setup.zeta_j(1)=1;
Setup.c_j = unifrnd(0,2,nmovies,ntaste)*10; %Movie position in taste space
Setup.c_j(1,1) = 1; % Normalize 1 movie
Setup.c_j(1,2) = 1; % Normalize 1 movie

% fix coefficients
Setup.betapar = [0.1,.5, .3];
Setup.gammapar = -rand(ntaste,1);
Setup.gammapar(1)=-1; %For now set to -1
Setup.gammapar(2)=-1; %For now set to -1
Setup.mu = unifrnd(0,2,nmarket,ntaste)*10;
Setup.mu(1,1)=5; %Normalize 1 market
Setup.mu(1,2)=5; %Normalize 1 market
Setup.sigma = unifrnd(0,2,nmarket,ntaste)+1;
Setup.sigma(1)=1;
...
%nparams = ntaste+nmovies*ntaste+2*nmarket*ntaste;
