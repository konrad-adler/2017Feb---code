...
%Setup.zeta_j(1)=1;
Setup.c_j = abs(normrnd(1,1,nmovies,ntaste))*10; %Movie position in taste space
Setup.c_j(1) = 1; % Normalize 1 movie

% fix coefficients
Setup.betapar = [1,5, 3];
Setup.gammapar = -rand(ntaste,1);
Setup.gammapar(1)=-2; %For now set to -1
Setup.mu = unifrnd(0,2,nmarket,ntaste)*10;
Setup.mu(1)=50; %Normalize 1 market
